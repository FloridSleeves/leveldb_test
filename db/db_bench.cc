...
#define PARAM_LEN (strlen(op->param))
#ifdef TIMER_LOG
...
// Trace file name
static const char* FLAGS_trace = NULL;
...
class Benchmark {
 ...
 public:
  ...
  int split_file_names(const char *file, char file_names[20][100]) {
	  ...
	  if(strlen(file)==0)
          {
	     fprintf(stderr,"Please set the system variable trace_file!\n");
	     exit(1);
	  }
  ...
  void parse_trace(const char *file, int tid) {
  	...
  	if(fp == NULL)
	{
	    fprintf(stderr,"\n%s not found",corresponding_file);
	}
  	curop = trace_ops[tid];
  	while((ret = getline(&buf, &bufsize, fp)) > 0) {
        ...
        char tmp[2000];
  		ret = sscanf(buf, "%c %s \n", &curop->cmd, curop->key);
        if(buf[1+1+strlen(curop->key)]!='\n') 
        {
            strcpy(curop->param,buf+1+1+strlen(curop->key)+1);	
            ret++;
        }
        assert(ret == 2 || ret == 3);
  ...
  Status perform_op(DB *db, struct trace_operation_t *op, int tid) {
	struct timeval start,end;
  	...
  	keylen = sprintf(keybuf, "%s", op->key);
	...
	gettimeofday(&start,NULL);
  	if (op->cmd == 'r') {
        ...
        std::string count_time;
  		...
  		status = db->Put(woptions, key, Slice(valuebuf, PARAM_LEN));
  		sassert(status.ok());
  		result.ycsbdata += keylen + PARAM_LEN;
  		result.kvdata += keylen + PARAM_LEN;
  		result.ycsb_i++;
  		result.kv_p++;
  	} else if (op->cmd == 'u') {
        ...
        if(status.ok())
        {
            WriteBatch batch;
            batch.Delete(key);
            batch.Put(key,Slice(valuebuf,strlen(value.c_str())));
  ...
  #define envinput(var, type) { int ret = sscanf(var, type, &var); assert(ret == 1);}
  ...
  void YCSB(ThreadState* thread) {
	...
	size_t bufsize=2000;
	struct trace_operation_t curop_inst;
    struct trace_operation_t *curop=&curop_inst;
	FILE *fp=fopen(FLAGS_trace,"r");
	char *buf=(char*)malloc(bufsize);
	int ret;
	int flag=getline(&buf,&bufsize,fp);
	...
	while(flag>0) {
		...
		if(!curop->cmd)
		    break;
		tmpstr.replace(0,2+strlen(curop->key)+1,"");
		strcpy(curop->param,tmpstr.c_str());
		if(strlen(curop->param)>0)
		    ret++;
		if(curop->cmd=='r'||curop->cmd=='d')
		{
		    assert(ret==2);  
  ...
  void ReliabilityCheck(ThreadState* thread) {
	...
	while (infile >> key) {
		...
		if (db_->Get(options, tempkey, &value).ok()) {
			found++;
		} else {
...
/*    const int size = 4096;
  ...
  void Open() {
    ...
    //std::string value;
    //std::string key("user2408371864701034737");
    //std::string count_time="";
    //db_->Get(ReadOptions(),key,&value,&count_time);
  }
...
int main(int argc, char** argv) {
  FLAGS_write_buffer_size = 16*1024*1024;
  FLAGS_open_files = leveldb::Options().max_open_files;
  FLAGS_block_size = 4*1024;
  FLAGS_cache_size = 64*1024;
  ...
  std::string db_str(argv[1]);
  FLAGS_db=db_str.c_str();
  std::string trace_str(argv[2]);
  FLAGS_trace=trace_str.c_str();
  
  std::string benchmark_str="ycsb";
  FLAGS_benchmarks=benchmark_str.c_str();
